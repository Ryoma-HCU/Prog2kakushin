...
2. 標本数が30未満または母標準偏差が不明の場合：t分布・\kai^{2}分布  
  (s:標本標準偏差, t:t分布確率密度, \kai^{2}:\kai^{2}分布確率密度)  
$$\bar{x}-t\frac{s}{\sqrt{n}} \leq \mu \leq \bar{x}+t\frac{s}{\sqrt{n}}$$
$$\frac{s\sqrt{n-1}}{\sqrt{upper\kai^{2}}} \leq \sigma \leq \frac{s\sqrt{n-1}}{\sqrt{lower\kai^{2}}}$$
...
Ex. 周波数帯域：1～10000 Hz　調査範囲：0～0.1  
