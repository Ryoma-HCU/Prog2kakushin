...
2. 標本数が30未満または母標準偏差が不明の場合：t分布・$\chi^{2}$分布  
  ($s$:標本標準偏差, $t$:t分布確率密度, $\chi^{2}$:$\chi^{2}$分布確率密度)  
$$\bar{x}-t\frac{s}{\sqrt{n}} \leq \mu \leq \bar{x}+t\frac{s}{\sqrt{n}}$$
$$s\sqrt{\frac{n-1}{upper\chi^{2}}} \leq \sigma \leq \frac{s\sqrt{n-1}}{\sqrt{lower\chi^{2}}}$$
...
またその講義でも扱っていない，標本数が少ないまたは母標準偏差が分からない場合の母集団推定に用いる$t$分布や$\chi^{2}$分布も自分で学習し，可能な限り計算式だけで導出できる様にプログラムした．  
